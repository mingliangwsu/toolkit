...
The outlet need elevation input as mode (to get cols and rows)
@author: liuming
"""
import sys 
import re
...
import re
if len(sys.argv) <= 1:
    print("Usage:" + sys.argv[0] + "<1gis_mode> <2output> <3value> <4type (int or float)>\n")
    sys.exit(0)
...
with open(sys.argv[1], 'r') as input_file, open(sys.argv[2], 'w') as output_file:
    ...
    for line in input_file:
        ...
        if elems[0] not in keys:
            ...
            for elem in elems:
                col_id += 1
                if abs(float(elem) - nodata) >= 0.00001:
                    out = value
                else:
                    out = nodata
                if sys.argv[4] == 'int':
                  output_file.write(str(int(out)) + ' ')
                else:
