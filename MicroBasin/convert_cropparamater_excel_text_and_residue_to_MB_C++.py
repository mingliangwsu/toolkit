...
if len(sys.argv) <= 1:
    print("Usage:" + sys.argv[0] + "<1input> <2output> <3mode> <4residue_parameter>\n")
    sys.exit(0)
...
residue_dic = dict()
with open(arguments[3], 'r') as file:
    line_id = 0
    for line in file:
        dlist = line.rstrip().split('\t')
        line_id +=1
        if dlist[0] != '' and line_id >= 9 and line_id <= 24:
            residue_dic[dlist[0]] = dlist

crop_file_name = arguments[0].rsplit('/', 1)[-1]
cropname = crop_file_name.rsplit('.', 1)[0]
residue_param_index_table = {
    'fast_cycling_top_fraction' : 1,
    'slow_cycling_top_fraction' : 2,
    'lignified_top_fraction' : 3,
    'fast_cycling_root_fraction' : 5,
    'slow_cycling_root_fraction' : 6,
    'lignified_root_fraction' : 7,
    'carbon_fraction' : 12,
    'stubble_area_covered_to_mass_ratio' : 13,
    'area_covered_to_mass_ratio' : 14
    }
...
with open(arguments[1], 'w') as file:
    # Read the entire content of the file
    with open(arguments[2], 'r') as mfile:
        for line in mfile:
            if len(line) > 0:
                if line[:1] != '[':
                    ...
                    if key in crop_paramater_values:
                        ...
                        if key in residue_param_index_table and cropname in residue_dic:
                            outline = key + '=' + residue_dic[cropname][residue_param_index_table[key]] + '\n'
                        else:
                            outline = line
                else:
                    outline = line
