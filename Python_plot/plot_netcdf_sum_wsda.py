...
if test != "kamiak":
    ...
    fname = '/home/liuming/mnt/hydronas3/LiuNas3/Project/USDA_Hydro/outnc_kamiak_field_irrig_EEFlux_AzeemMethod.nc'
...
#vars = ["ETRF","ETR","ET","NDVI","CS_SOILEVAP","CS_TRANS","CS_ET","CS_ET0","CS_IRRIG","CS_BIOMASS","CS_NDVI_FIT","CS_DAE"]
#vars = ["ETR","ET","CS_SOILEVAP","CS_TRANS","CS_ET","CS_ET0","CS_IRRIG"]
vars = ["CS_TRANS","CS_IRRIG","CS_SOILEVAP","ET","CS_ET"]
for var in vars:
    for cdl in cdls:
        ...
        ds_avg_map = cdl_ds.sum(dim=['time'])    
        axes[0].set_ylim([0,10])
        ds_timeseries[var].plot(ax=axes[0])
        #ds_avg_map[var].plot(ax=axes[1],norm=norm_NDVI,cmap='YlGn')
        ds_avg_map[var].plot(ax=axes[1],norm=norm_ET,cmap='YlGn')
        if test != "kamiak":
            outimage = outdir + "/fig_sum_" + cdls[cdl] + "_" + var + ".png"
        else:
            outimage = outdir + "/kamiak_fig_sum" + cdls[cdl] + "_" + var + ".png"
