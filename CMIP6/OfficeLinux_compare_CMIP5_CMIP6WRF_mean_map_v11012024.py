...
if tmap == 'GRIDMET':
    ...
    for var in varss:
        ...
        plt.title(f'GridMet {var} {period_name[1]}', fontsize=15)
    ...
    for gcm in GCMS:
        ...
        if not os.path.exists(filename):
            ...
            if not os.path.exists(filename):
                ...
                sys.exit()
        idx = 0
        for chunk in pd.read_csv(f'{gridmetmaca_path}/PNW_{gcm}_VIC_GCM_yearmonth_mean_grid.csv', chunksize=chunksize):
            # Process each chunk
            #print(chunk.head())
