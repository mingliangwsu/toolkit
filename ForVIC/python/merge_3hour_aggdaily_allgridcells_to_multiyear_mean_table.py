...
files = ["flux_45.53125_-122.28125","flux_47.59375_-121.53125"]
...
for mode in modes:
    ...
    for simindex in range(0,40):
        print(simindex)
        #loop all files
        dpath = vic_path[mode] + "/sim" + str(simindex)
        ...
        #year_month[mode] = year_month[mode].drop(columns=["DAY"])
                ...
                yearsum[mode] = data[mode][(data[mode]["YEAR"] >= 1983) & (data[mode]["YEAR"] <= 2008)].groupby(["YEAR"]).sum().reset_index()
                ...
                if index == 0:
                    merged = yearavg[mode].iloc[0:0]
                merged = merged.append(yearavg[mode])
                index += 1
    ...
    #plt.show()
    #index += 1
outfilename = outfig_path + "/allsim_mean_1983_2008.csv"
merged.to_csv(outfilename,index=False)