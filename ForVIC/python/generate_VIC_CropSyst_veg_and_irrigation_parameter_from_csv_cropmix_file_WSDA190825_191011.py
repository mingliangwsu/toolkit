...
path_dir = "/home/liuming/Projects/WSU_BPA/CRB_Datasets/LandUseAndIrrigation/WSDA/"
...
fraction_file = indir + "/vic_crop_irrigation_area_fraction_remove_quote.txt"
cell_list = indir + "/vic_list_in_wa.csv"
...
irrigation_list = [
                ...
                "Big Gun/Drip",
                "Big Gun/Sprinkler",
                "Big Gun/Sprinkler/Wheel Line",
                "Big Gun/Wheel Line",
                "Center Pivot",
                "Center Pivot/Drip",
                "Center Pivot/Drip/Sprinkler",
                "Center Pivot/None",
                "Center Pivot/Rill",
                "Center Pivot/Rill/Sprinkler",
                "Center Pivot/Rill/Wheel Line",
...
for cell in allcell:
    ...
    for veg in out_veg_list:
        tcrop[veg] = 0.0
    cell_crop_fraction[cell] = tcrop
print("initial cell crop fraction\n")
    ...
    for line in f:
        if "GRID_CODE" not in line:
            ...
            if (len(a) > 0 and a[0] in allcell):
                if a[1] in cdl_to_vic_dic and a[2] in irrigation_list:
                    outveg = cdl_to_vic_dic[a[1]]
                    if outveg in out_veg_list:
                        cell_crop_fraction[a[0]][outveg] += float(a[4])
...
for cell in allcell:
    total_area = 0
    total_veg_num = 0
    for ov in cell_crop_fraction[cell]:
        total_area += cell_crop_fraction[cell][ov]
        if (cell_crop_fraction[cell][ov] > tol):
            total_veg_num += 1
    if total_veg_num >= 1:
        line = cell + " " + str(total_veg_num) + "\n"
        recorded_cell.append(cell)
        outfile_veg.write(line)
    for ov in cell_crop_fraction[cell]:
        if (cell_crop_fraction[cell][ov] > tol):
            if ov in paramter_dic:
                outp = paramter_dic[ov]
            else:
                outp = default_veg_parameter
            line = "   " + ov + " " + str('%.5f' % cell_crop_fraction[cell][ov]) + " " + outp + "\n"
            outfile_veg.write(line)
