...
vic_modis_nlcd = inout_data_path + "nlcdmod2.csv"
vic_imp = inout_data_path + "impvic.csv"
vic_nlcd = inout_data_path + "nlcdvic.csv"

outdir = "/home/liuming/temp/temp"
out_veg_file = "landcover_fraction_nlcd_modis.txt"
out_log = "landcover_fraction_nlcd_modis_log.txt"
...
for grid in vic_area_num30pix_dic:
    ...
    total_land = 0
    total_developed = 0
    total_impervious = vic_imp_persum[grid] * 0.01  #number of 30mX30m pixel
    for nlcd in nlcd_type_list:
        if nlcd != "11":
            total_land += vic_nlcd_num30pix_dic[grid][nlcd]
        if nlcd in nlcd_delveloped:
            total_developed += vic_nlcd_num30pix_dic[grid][nlcd]
        elif nlcd == "41" or nlcd == "42":
            sum_deciduous = modis_forest_fraction_dic[a[2]]["deciduous"]["needle"] + modis_forest_fraction_dic[a[2]]["deciduous"]["broad"]
            if sum_deciduous >= 1:
                frac_dec_needle = float(modis_forest_fraction_dic[a[2]]["deciduous"]["needle"] / sum_deciduous)
            else:
                frac_dec_needle = default_modis_forest_fraction_dic["3"] / (default_modis_forest_fraction_dic["3"] + default_modis_forest_fraction_dic["4"])
            frac_dec_broad = 1.0 - frac_dec_needle
            
            sum_evergreen = modis_forest_fraction_dic[a[2]]["evergreen"]["needle"] + modis_forest_fraction_dic[a[2]]["evergreen"]["broad"]
            if sum_evergreen >= 1:
                frac_evg_needle = float(modis_forest_fraction_dic[a[2]]["evergreen"]["needle"] / sum_evergreen)
            else:
                frac_evg_needle = default_modis_forest_fraction_dic["1"] / (default_modis_forest_fraction_dic["1"] + default_modis_forest_fraction_dic["2"])
            frac_evg_broad = 1.0 - frac_evg_needle
            
            if nlcd == "41":
                vic_class_num30pix_dic[grid]["3"] += vic_nlcd_num30pix_dic[grid][nlcd] * frac_dec_needle
                vic_class_num30pix_dic[grid]["4"] += vic_nlcd_num30pix_dic[grid][nlcd] * frac_dec_broad
            elif nlcd == "42":
                vic_class_num30pix_dic[grid]["1"] += vic_nlcd_num30pix_dic[grid][nlcd] * frac_evg_needle
                vic_class_num30pix_dic[grid]["2"] += vic_nlcd_num30pix_dic[grid][nlcd] * frac_evg_broad
            ...
            vic_class_num30pix_dic[grid]["10"] += vic_nlcd_num30pix_dic[grid][nlcd]
        elif nlcd == "12" or nlcd == "31":
            vic_class_num30pix_dic[grid]["14"] += vic_nlcd_num30pix_dic[grid][nlcd]
    if total_developed >= total_impervious:
        vic_class_num30pix_dic[grid]["12"] += total_impervious
        vic_class_num30pix_dic[grid]["13"] += total_developed - total_impervious
    else:
...
for grid in sorted(vic_area_num30pix_dic, key=sortkey, reverse=False):
    ...
    if vic_area_num30pix_dic[grid] >= (32543 * 0.9):
        if total_area / vic_area_num30pix_dic[grid] > 0.000001:
            outfile_veg.write(grid)
