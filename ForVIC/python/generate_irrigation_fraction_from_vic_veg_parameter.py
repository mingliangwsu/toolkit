...
out_irrigated = datapath + "irrigated_fraction.txt"
#out_cropall = datapath + "all_crops_gridcells.txt"

irrigated = dict() #[gridcell] list
fraction = dict() #[gridcell_id]
allcrop_grid = list()
...
with open(irrigation_file) as f:
    for line in f:
        ...
        if a[1].isdigit():
            if int(a[1]) >= 1:
                gridid = a[0]
                if gridid not in irrigated:
                    irrigated[gridid] = list()
        else:
            irrigated[gridid].append(a[0])
...
with open(veg_file) as f:
    for line in f:
        ...
        if len(a) == 8:
            cropcodeint = int(a[0])
            if cropcodeint > 20:
                if grid not in fraction:
                    fraction[grid] = 0.0
                if grid in irrigated:
                    if str(cropcodeint) in irrigated[grid]:
                        fraction[grid] += float(a[1])
with open(out_irrigated,"w") as fout:
    fout.write("GRID_CODE,irrif\n")
    for grid in fraction:
        outline = grid + "," + str(fraction[grid]) + "\n"
        fout.write(outline)
