...
cdl_vegparameter_file = datapath + "us_veg_parameter.txt"
...
#20210212 removed "4004","4007","4008","4100","4101","4102","6002","7106","7207","7720","7801","7806","7807","8001","8002","8802","8807","8809","8811","8815","8817","8820","8826","8828","8831","8832","8834","8839","8841","8904","9204","9205","9208"
Always_irrigation = ["102","103","107","198","1401","1402","1403","1407","1409"
                     ,"1410","1411","2002","2207","2502","2505"]
...
#read cdl fraction information
WheatFraction = dict() #[grid][wheat_type:win,spg,all] 
winwheat = ["4005","14005"]
spgwheat = ["4006","14006"]
with open(cdl_vegparameter_file) as f:   
    current_line = 0
    nextgrid_line = 0
    for line in f:
        a = line.rstrip().split()
        if current_line == nextgrid_line: #gridid num_veg
            ...
            if len(a) == 8:
                vegcode = a[0] 
                if vegcode in winwheat or vegcode in spgwheat:
                    if grid not in WheatFraction:
                        WheatFraction[grid] = dict()
                        WheatFraction[grid]['win'] = 0.0
                        WheatFraction[grid]['spg'] = 0.0
                        WheatFraction[grid]['all'] = 0.0
                    if vegcode in winwheat:
                        WheatFraction[grid]['win'] += float(a[1])
                    else:
                        WheatFraction[grid]['spg'] += float(a[1])
                    WheatFraction[grid]['all'] += float(a[1])
        current_line += 1
...
for region in regions:
                ...
                if len(a) > 0:
                    ...
                    if int(grid) > 0 and fraction > 0.00001:
                      if crop in wsda_to_vic_dic: #crop
                          ...
                          if vic_crop == '4005': #seperate wheat into spring and winter wheat based on cdl types
                              winw_f = 1.0  #fraction of winter wheat in all wheat
                              if grid in WheatFraction:
                                  if WheatFraction[grid]['all'] > 0.00001:
                                      winw_f = WheatFraction[grid]['win'] / WheatFraction[grid]['all']
                              for wheat in ['4005','4006']:
                                  wfraction = 0.0
                                  if wheat == '4005':
                                      wfraction = winw_f * fraction
                                  else:
                                      wfraction = (1.0 - winw_f) * fraction
                                  if wfraction > 0.00001:
                                      if wheat not in crop_irrig_fraction[grid]:
                                          crop_irrig_fraction[grid][wheat] = dict()
                                      if irr in irrigation_list:
                                          irri_type = irrigation_list[irr]
                                      else:
                                          irri_type = "No_irrigation"
                                      if wheat in Always_irrigation and irri_type == "No_irrigation":
                                          irri_type = default_crop_irrigation_type
                                      if irri_type not in crop_irrig_fraction[grid][wheat]:
                                          crop_irrig_fraction[grid][wheat][irri_type] = 0.0
                                      crop_irrig_fraction[grid][wheat][irri_type] += wfraction
                          else: #other non-wheat crop
    ...
    for grid in sorted(vicveg_veg_parameter, key=sortkey, reverse=False):
        ...
        for crop in croptype:
            if crop in vicveg_veg_parameter[grid]: 
                cropnum += 1
                cropfraction += vicveg_veg_fraction[grid][crop]
        if cropnum > 0:
        ...
        if grid in crop_fraction:
            for crop in sorted(crop_fraction[grid], key=sortkey, reverse=False):
                ...
                if croptype[0] in vicveg_veg_lai[grid]:
                    ...
                    fout_veg.write(vicveg_veg_lai[grid][croptype[1]])
                else:
