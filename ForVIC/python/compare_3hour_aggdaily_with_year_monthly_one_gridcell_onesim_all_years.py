...
for mode in modes:
    ...
    year_month[mode] = data[mode].groupby(["YEAR","MONTH"]).sum().reset_index()
    ...
    ax = year_month[mode]["OUT_EVAP"].plot()
    ax.set_ylim(0, 120)
    ax.set(xlabel="Months since 1981/01",ylabel="ET (mm/month)",title=titlemessage)
    
    ax2 = year_month[mode]["tot_runoff"].plot(secondary_y=True)
    ax2.set_ylim(0, 3000)
