...
outputdir = '/home/liuming/mnt/hydronas3/Projects/WWS_Oregon/Output_realrun_11072023/sim_obs_streamflows'
...
if True:
    for landcover_year in landcover_years:
        for print_start in print_start_ends:
            ...
            if "year_x" in alldata.columns:
                if "year" not in alldata.columns:
                    alldata = alldata.rename(columns={"year_x" : "year","month_x" : "month","day_x" : "day"})
                else:
                    alldata = alldata.drop(columns=["year_x","month_x","day_x"])
            if "year_y" in alldata.columns:
                if "year" not in alldata.columns:
                    alldata = alldata.rename(columns={"year_y" : "year","month_y" : "month","day_y" : "day"})
                else:
                    alldata = alldata.drop(columns=["year_y","month_y","day_y"])
            ...
            for plot_time in plot_times:
              ...
              for var in climate_vars: #climate_vars:
                ...
                outdata_file = f'{outputdir}/{var}_{plot_time}_landcoveryear_{landcover_year}_f{print_start}_t{print_start_ends[print_start]}.csv'
                alldata.to_csv(outdata_file,index=False)
