...
#average 1985-1989
map_lai_annual_1985_1989 = map_lai_annual[(map_lai_annual.year >= 1985) & (map_lai_annual.year <= 1989)].groupby(['patchID']).agg(
...
for year in range(1990,2016,1):
    for consumption in consumption_list:
        for mort in mort_list:
            t =  map_lai_annual[(map_lai_annual.year == year) & (map_lai_annual.consumption == float(consumption)) & (map_lai_annual.mort == float(mort))]
            if len(t) > 0:
                print(consumption + " " + mort + " " + str(year))
                fig = plt.figure(figsize=(20,16)) 
                p1 = plt.scatter(t.x, t.y,
                                 c=t.lai_max,cmap='YlGn',vmin=0, vmax=6,s=30,marker='s')
                fig.colorbar(p1, label='LAI')

                plt.title("LAI " + str(year) + "    Mort:" + mort + "    Consumption Coef:" + consumption,fontsize=18)
                plt.savefig(path + "/map_LAI_"+ str(year) + "_Mort_" + mort + "_Consumption_" + consumption + ".png")
                plt.close()
