...
fig.colorbar(p1, label='Litter C')

plt.title("Mean Litter C 1985-1989",fontsize=18)
plt.savefig(path + "/mean_litterc_1985_1989.png")
plt.close()
        ...
        for mort in mort_list:
            ...
            if len(t) > 0:
                ...
                p1 = plt.scatter(t.x, t.y,
                                 c=t.litter_c,cmap='YlGn',vmin=0, vmax=50,s=30,marker='s')
                fig.colorbar(p1, label='Litter C')

                plt.title("Litter C " + str(year) + "    Mort:" + mort + "    Consumption Coef:" + consumption,fontsize=18)
                plt.savefig(path + "/map_litterc_"+ str(year) + "_Mort_" + mort + "_Consumption_" + consumption + ".png")
                plt.close()
