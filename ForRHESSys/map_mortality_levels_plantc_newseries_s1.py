...
map_c_annual = pd.read_csv(path + "/s1_map_c_annual.csv")

#average 1985-1989
map_annual_1985_1989 = map_c_annual[(map_c_annual.year >= 1985) & (map_c_annual.year <= 1989) & (map_c_annual.ps <= 0.001)].groupby(['patchID']).agg(
        avgv=('plant_c', 'mean'),
        x=('x', 'mean'),
        y=('y', 'mean'),
...
cbar = fig.colorbar(p1, label='Plant C')
...
for year in range(1985,2018,1):
    #for consumption in consumption_list:
        for mort in mort_list:
            t =  map_c_annual[(map_c_annual.year == year) & (map_c_annual.ps == float(mort))]
            if len(t) > 0:
                ...
                for t in cbar.ax.get_yticklabels():
                    t.set_fontsize(16)
                plt.title("Plant C " + str(year) + "    pspread:" + mort,fontsize=18)
                plt.savefig(path + "/map_plantc_" + + str(year) + "_ps_" + mort + ".png")
                plt.close()
