...
for target_year in ["1994","1999"]:
    ...
    mean_sts = sel_sts.groupby(['year'], as_index=False).mean()
    mean_sts = mean_sts[['year',target_var]]
    #if target_year == '1999':
    #    tyear = 1998
    #else:
...
#merge 
alldf = pd.merge(df['1994'],df['1999'],on=['after_burn'])
...
if True:
  ...
  for var in ['B1994','B1999']:
      ...
      plt.plot(sub_data['after_burn'], sub_data[var], label=f'{plotvar[var]}', color=climate_color[var],linestyle=climate_style[var],linewidth=linew)
...
plt.gca().set_ylim(bottom=60,top=90)
...
outpng = f'{figure_outdir}/fig_mean_canopy_cover_shift.png'
