...
sel_sts[target_var] = sel_sts.apply(lai_to_cc, axis=1)
...
for year in year_list:
    ...
    for var in map_vars:
        ...
        tmap = t.plot(kind='scatter', 
               ...
               ax=ax)
        tmap.collections[0].colorbar.remove()
        colorbar = plt.colorbar(tmap.collections[0])
        colorbar.set_label('Canopy Cover (%)')
        ax.set_title(f'B{target_year} Canopy Cover (%) ({year})')        
        ax.set_xlabel("Col")
        ax.set_ylabel("Row")
        ...
        outpng = f'{figure_outdir}/newmap_B{target_year}_canopy_cover_{year}.png'
