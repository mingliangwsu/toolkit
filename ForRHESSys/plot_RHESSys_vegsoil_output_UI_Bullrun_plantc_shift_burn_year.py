...
outunit = "KgC/m2"
#outunit = "%"
plot_ouput_var = "PlantC"
plot_title = "PlantC (kg/m2)"
#if output_var = "lai":
...
for c_burnt_year in byears:
    RHESSys_outputdir = model_output_root + "/c" + c_burnt_year 
    #os.chdir(droot)
    
    if True:
        #output_pre = RHESSys_outputdir + '/real_run_' + clim
        fname_basin_daily_out = RHESSys_outputdir + '/c' + c_burnt_year + "_" + output_file
...
if True:
    ...
    if "year_y" in alldata.columns:
        #alldata = alldata.drop(columns=["year_y","month_y","day_y"])
        alldata.rename(columns={"year_y": "year"}, inplace=True)
    if "year_x" in alldata.columns:
        #alldata = alldata.drop(columns=["year_x","month_x","day_x"])
        alldata.rename(columns={"year_x": "year"}, inplace=True)
    ...
    for plot_time in plot_times:
      ...
      plt.gca().set_ylim(bottom=15,top=35)
