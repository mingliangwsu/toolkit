...
map_lai_annual = pd.read_csv(path + "/s2_map_lai_annual.csv")


#map_c_annual = pd.read_csv(path + "/map_c_annual.csv")

#average 1985-1989
#map_lai_annual_1980_1984 = map_lai_annual[(map_lai_annual.year >= 1980) & (map_lai_annual.year <= 1984) & (map_lai_annual.ps <= 0.001)].groupby(['patchID']).agg(
...
for year in range(1985,2018,1):
    #for consumption in consumption_list:
        for mort in mort_list:
            t =  map_lai_annual[(map_lai_annual.year == year) & (map_lai_annual.undermort == float(mort))]
            if len(t) > 0:
                ...
                plt.title("LAI " + str(year) + "    undermort:" + mort,fontsize=18)
                plt.savefig(path + "/map_LAI_"+ str(year) + "_undermort_" + mort + ".png")
                plt.close()
