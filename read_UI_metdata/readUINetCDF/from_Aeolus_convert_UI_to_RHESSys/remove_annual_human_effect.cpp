...
int get_obs_met_year(const int target_year);
int get_annual_adjustment(const Adj_records *adjdata, const int year,
  double &tmax, double &tmin, double &rmax, double &rmin, double &sph);
...
int main(int argc, char *argv[])
{
    ...
    int shift_days = get_days_since_1900(0,start_year);
    for (int year = start_year; year <= end_year; year++) {
        size_t start[3],count[3];
        int obs_met_year = get_obs_met_year(year);
        bool leapyear_realyear = LEAPYR(year);
        bool leapyear_metyear = LEAPYR(obs_met_year);

        double obs_adj_tmax,obs_adj_tmin,obs_adj_rmax,obs_adj_rmin,obs_adj_sph;
        double rel_adj_tmax,rel_adj_tmin,rel_adj_rmax,rel_adj_rmin,rel_adj_sph;
        get_annual_adjustment(adjdata,obs_met_year,obs_adj_tmax,obs_adj_tmin,obs_adj_rmax,obs_adj_rmin,obs_adj_sph);
        get_annual_adjustment(adjdata,year,rel_adj_tmax,rel_adj_tmin,rel_adj_rmax,rel_adj_rmin,rel_adj_sph);

        double adj_tmax = obs_adj_tmax - rel_adj_tmax;
        double adj_tmin = obs_adj_tmin - rel_adj_tmin;
        double adj_rmax = obs_adj_rmax - rel_adj_rmax;
        double adj_rmin = obs_adj_rmin - rel_adj_rmin;
        double adj_sph = obs_adj_sph - rel_adj_sph;


        size_t tdays = leapyear_metyear ? 366 : 365;
        size_t rel_tdays = leapyear_realyear ? 366 : 365;
        start[0] = 0;
        start[1] = 0;        //lat
        start[2] = 0;        //lont
        std::cout<<"Year:"<<year<<" obs_met_year:"<<obs_met_year<<std::endl;
        for (int dvar = start_var; dvar <= end_var; dvar++) {
            std::stringstream ss;
            ss<<netcdf_folder<<varfile_prefix[dvar]<<"_"<<obs_met_year<<".nc";
            int temp_ncid;
            int var_id;
        ...
        #pragma omp parallel for
            for (int ilat = 0; ilat < nlat; ++ilat) {
                ...
                for (int ilon = 0; ilon < nlont; ++ilon) {
                    ...
                    for (int relday = 0; relday < rel_tdays; ++relday) {
                        int day;
                        if (!leapyear_realyear) day = relday;
                        else {
                          if (leapyear_metyear) day = relday;
                          else {
                            if (relday == 365) day = 364;
                            else day = relday;
                          }
                        }
                        ...
                        int out_time_index = get_days_since_1900(relday,year) - shift_days;
                        ...
                        if (out_all_index != -1) {
                            ...
                            case TMAX:
                                 ...
                                 if (remove_human_effect) out_tmax[out_all_index] -= adj_tmax;
                                 //add_valid_value(out_tmax[out_all_index],-9999.0,mean[day],number[day]);
                                 break;
                            case TMIN:
                                 out_tmin[out_all_index] = tmin[iindex];
                                 if (remove_human_effect) out_tmin[out_all_index] -= adj_tmin;
                            ...
                            case SPH:
                                 ...
                                 if (remove_human_effect) {out_sph[out_all_index] -= adj_sph;
                            ...
                            case RMAX:
                                 ...
                                 if (remove_human_effect) out_rmax[out_all_index] -= adj_rmax;
                                 break;
                            case RMIN:
                                 out_rmin[out_all_index] = rmin[iindex];
                                 if (remove_human_effect) out_rmin[out_all_index] -= adj_rmin;
                                 break;
#endif
                            }  //switch
                        }  //out_all_index
...
int get_obs_met_year(const int target_year)
{
  if (target_year >= 1979) return target_year;
  else if (target_year >= 1916 && target_year <= 1940) return 1992 + target_year - 1916;
  else if (target_year >= 1941 && target_year <= 1978) return 1979 + target_year - 1941;
  else {
    std::cerr << "Not right target_year!\n";
    exit(0);
  }
  return 0;
}
  ...
  double &tmax, double &tmin, double &rmax, double &rmin, double &sph)
{
  ...
  double srmin = 0;
  double ssph = 0;
  int index = (year - START_YEAR) * 12;
  for (int month = 0; month < 12; month++) {
    int mon_index = index + month;
    if (adjdata[mon_index].year != year) {
      std::cerr << "Error: the time in Adjustment data does not match!\n";
      exit(0);
    }
    stmax += adjdata[mon_index].tmax / 12.0;
    stmin += adjdata[mon_index].tmin / 12.0;
    srmax += adjdata[mon_index].rmax / 12.0;
    srmin += adjdata[mon_index].rmin / 12.0;
    ssph += adjdata[mon_index].sph / 12.0;
  }
  return 0;
}
